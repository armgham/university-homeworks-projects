...
public class AddDoctorGUI extends JFrame implements ActionListener {
	 ...
	 * @param dG
	 */
	public AddDoctorGUI(DoctorGUI dG) { // yek safhe baraye gereftane vorudi
		super("add new doctor");
		this.dG = dG;
		setSize(320, 500);
		...
		bOK.addActionListener(new ActionListener() {//bar asase vorudi docotor misazad
			@Override
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!tID.getText().equals(""))
					try {
						...
						if(Doctor.getAllIds().contains(id))
				...
				if(id.equals(0))
					new Doctor(firstName, lastName);
				else if(gender.equals("") && birthDate.equals(""))
					new Doctor(firstName, lastName, id);
				else
					new Doctor(firstName, lastName, gender, birthDate, id);
